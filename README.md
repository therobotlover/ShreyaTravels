...
Affordable premium travel booking MVP with OTP-only auth, hold-or-pay booking flow, and bKash payment scaffolding. Designed for Railway.app deployment with Docker.

## Live Demo

Live link - https://shreyatravels-production.up.railway.app/
![Live Demo](screenshots/live-page.png)
Have patient due to lack of resources, it can get sleep due to inactivity; if you are first visitor within 30 minutes, the website can slow down. Wait for few minutes after first hit.
...
4. Set `APP_KEY` (generate with `php artisan key:generate --show`).
5. Ensure `DB_*` values match the Railway MySQL service.
6. Deploy. The container runs migrations and caches on boot via `start.sh`.
